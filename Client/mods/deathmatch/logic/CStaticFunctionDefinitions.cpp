...
int CStaticFunctionDefinitions::CreateCullZone(float centerX, float centerY, float centerZ, float length, float bottom, float width, float top, unsigned int flags)
{
    ...
    auto attributeZones = reinterpret_cast<Zone*>(0xC81F50);
    auto& numAttributeZones = *reinterpret_cast<int*>(0xC87AC8);

    if (numAttributeZones >= 1300)
        return -1;

    Zone zone{};
    zone.minX = centerX - width;
    zone.maxX = centerX + width;
    zone.minY = centerY - length;
    zone.maxY = centerY + length;
    zone.minZ = bottom;
    zone.maxZ = top;
    zone.flags = flags;

    attributeZones[numAttributeZones] = zone;
    ++numAttributeZones;

    if (!(flags & 0x20))
    {
        auto cullZones = reinterpret_cast<Zone*>(0xBA1900);
        auto& numCullZones = *reinterpret_cast<int*>(0xBA18FC);
        if (numCullZones < 64)
        {
            cullZones[numCullZones] = zone;
            ++numCullZones;
        }
    }

    return numAttributeZones - 1;
}
...
bool CStaticFunctionDefinitions::RemoveCullZone(int id)
{
    ...
    auto attributeZones = reinterpret_cast<Zone*>(0xC81F50);
    auto& numAttributeZones = *reinterpret_cast<int*>(0xC87AC8);
    if (id >= numAttributeZones)
        return false;

    Zone removed = attributeZones[id];
    if (id != numAttributeZones - 1)
    {
        attributeZones[id] = attributeZones[numAttributeZones - 1];
    }
    --numAttributeZones;

    if (!(removed.flags & 0x20))
    {
        auto cullZones = reinterpret_cast<Zone*>(0xBA1900);
        auto& numCullZones = *reinterpret_cast<int*>(0xBA18FC);
        for (int i = 0; i < numCullZones; ++i)
        {
            if (memcmp(&cullZones[i], &removed, sizeof(Zone)) == 0)
            {
                if (i != numCullZones - 1)
                    cullZones[i] = cullZones[numCullZones - 1];
                --numCullZones;
                break;
            }
        }
